...
def validate(data, type=None, lBound=None, uBound=None, length=0, format=None, existence=[]):
    ...
    if format != None:
        if format == "date":
            if not (int(data[2:4]) >= 1 and int(data[2:4]) <= 12):
                return False 
            elif not (int(data[4:6]) >= 1 and int(data[4:6]) <= 31):
                return False
        elif re.search(format, data) == None:                             # Format Check (using RegEx)
            return False                   

    if existence != []:                                                 # Existence Check
        if not data in existence:
            return False
